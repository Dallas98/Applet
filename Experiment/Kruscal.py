...
vexs = array([[0, 10, 7, INFINITY, INFINITY, 11, INFINITY, INFINITY, INFINITY],  # 邻接矩阵
              ...
              [7, 18, 0, 22, INFINITY, INFINITY, INFINITY, INFINITY, 8],
...
print("图的节点数：", lengthVex)
beginEdge = []
endEdge = []
weight = []
group = []
for i in range(lengthVex):  # 生成边集数组
    group.append([i])
...
print("边的条数：", lengthEdge)
sum = 0
for i in range(lengthEdge):  # 遍历每条边
    I = (argsort(weight))[0]
    ...
    if m != n:  # 判断当前这条边是否属于不同的连通分量，如果是，将其合并
        ...
        group[n] = []
        sum = sum + weight[I]
        print(beginEdge[I], "--->", endEdge[I], ":", weight[I])
    del weight[I]  # 删除遍历过的边以及顶点
    del beginEdge[I]
    del endEdge[I]
print("最小代价生成树的长度为:", sum)
