...
while True:
    # 创建一个1帧的视频框架   返回两个参数，ret是true/false代表有没有读到图片，frame是截取当前一帧的图片
    ret, frame = video_capture.read()
    # 将视频的大小调整为1/4大小以获得更快的人脸识别处理  显示出来的还是原来的1帧视频
    ...
    for (top, right, bottom, left), name in zip(face_locations, face_names):
        # zip可作为迭代的参数  对象中对应的元素打包成一个个元组，然后返回由这些元组组成的列表，返回列表长度与最短的对象相同  上下左右迭代成locations，encoding迭代成encodings
        # 放大4倍 还原成像
        top *= 4
        right *= 4
        bottom *= 4
        ...
        # 画出人脸矩形框    第一个参数是图片 第二个（left, top）是矩阵的左上点坐标  第三个（right, bottom）是矩阵的右下点坐标  第三个是画线对应的bgr颜色  第四个是所画的线的宽度
